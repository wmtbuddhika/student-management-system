<?php 
	...
	if(!empty($enter_by) || $enter_by != NULL){
        ...
        if($meeting_id > 0){
            $query = "UPDATE meeting SET schedule_date = '$dom',comment = '$meeting_message',allocation_id = '$groupId',
                        title = '$meeting_topic',status = '$status',entered_by = '$enter_by' WHERE id = $meeting_id";
            $query_execute = mysqli_query($con, $query);
        } else {
            $query = "INSERT INTO meeting (schedule_date,comment,allocation_id,title,status,entered_by)
			VALUES ('$dom','$meeting_message','$groupId','$meeting_topic','$status','$enter_by')";
